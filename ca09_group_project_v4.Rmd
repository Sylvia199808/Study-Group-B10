...
#we group the property types into 3 categories according to frequency distribution to only control for the largest subgroups
barcelona$property <- if (barcelona$property_type == "Entire rental unit") {"Full House"
  } else if (barcelona$property_type %in% c("Private room in rental unit", "Private room in Condominium (condo)", "Private room in residential home", "Private room in hostel", "Private room in guest suite", "Private room in bed and breakfast", "Private room in loft", "Private room in serviced apartment", "Private room"))
  {"Private room"
    } else {"Others"}
...
categorical_specific_longer <- property_specific %>% 
  select(bathrooms_per_guest,
         bedrooms_per_guest,
         bathrooms,
         bedrooms,
...
ggplot(data = barcelona_price, aes(price_4_nights)) +
  geom_density()

ggplot(data = barcelona_price, aes(logprice_4_nights)) +
  geom_density()
...
```{r}
model4 <- lm(logprice_4_nights ~ prop_type_simplified + number_of_reviews + review_scores_rating + room_type + bedrooms + accommodates + host_is_superhost, 
             data = barcelona_analysis)
msummary(model4)
car::vif(model4)
autoplot(model4)
...
```{r}
model5 <- lm(logprice_4_nights ~ prop_type_simplified + number_of_reviews + review_scores_rating + room_type + bedrooms + accommodates + host_is_superhost + instant_bookable, 
             data = barcelona_analysis)
msummary(model5)
car::vif(model5)
autoplot(model5)
...
```{r}
model6 <- lm(logprice_4_nights ~ prop_type_simplified + number_of_reviews + review_scores_rating + room_type + bedrooms + accommodates + host_is_superhost + instant_bookable + neigbourhood_simplified, 
             data = barcelona_analysis)
msummary(model6)
car::vif(model6)
autoplot(model6)
...
```{r}
model7 <- lm(logprice_4_nights ~ prop_type_simplified + number_of_reviews + review_scores_rating + room_type + bedrooms + accommodates + host_is_superhost + instant_bookable + neigbourhood_simplified + availability_30 + reviews_per_month, 
             data = barcelona_analysis)
msummary(model7)
car::vif(model7)
autoplot(model7)
