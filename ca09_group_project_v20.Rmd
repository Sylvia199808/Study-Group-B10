...
Difference of RMSE between raining data set and testing data set is not very large and the training Rsquared is higher than the testing one.
...
The adjusted Rsquared of model2 is 0.4718, which means that the selected variables are able to explain 47.18%% of the variation in prices. This is an improvement compared to model1 (43.95%). Residuals of model2 do not obey Normal Distribution well, we can see some patterns as for model1, which let us think that we might still be missing key explanatory variables for the prediction of the price.
...
Difference of RMSE between raining data set and testing data set is very small and the testing Rsquared is higher than the training one, which might imply slight overfitting, i.e. our model is becoming very complex.
...
```
Difference of RMSE between raining data set and testing data set is not very large and the training Rsquared is now higher than the testing one, which is a good sign.
